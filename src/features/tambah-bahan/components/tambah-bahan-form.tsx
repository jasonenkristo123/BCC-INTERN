...
import { KategoriBahan, SatuanBahan, TTambahBahanScema, TambahBahanScema } from "../schemas/tambah-bahan-schema";
        ...
        <div className="py-10 px-10 w-full min-h-screen bg-skyblue">
            <div className="mt-20">
                <div className="flex gap-2 text-[16px] font-roboto-600">
                    <a href="/bahan-saya">
                        <p className="text-hitamdikit/50">
                            Bahan Saya
                        </p>
                    </a>
                    <p className="text-hitamdikit">
                        {" > "} Tambah Bahan
                    </p>
                </div>
                <div className="mt-3">
                ...
                <form className="mt-8" onSubmit={handleSubmit(onSubmit)}>
                    <div className="bg-white rounded-xl p-10 shadow-lg">
                        ...
                        <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-[18px] ">
                            <div>
                                <label htmlFor="nama" className="text-sm sm:text-base md:text-lg lg:text-xl font-roboto-400 text-hitamdikit block mt-2">
                            ...
                            <div>
                                <label htmlFor="kategori" className="text-sm sm:text-base md:text-lg lg:text-xl font-roboto-400 text-hitamdikit block mt-2">
                            ...
                            <div>
                                <label htmlFor="tanggal-beli" className="text-sm sm:text-base md:text-lg lg:text-xl font-roboto-400 text-hitamdikit block mt-2">
                                    Tanggal Beli <span className="text-merah">*</span>
                                </label>
                                <input
                                    type="date"
                                    className="w-full  border border-hitamdikit/30 rounded-xl px-3 py-2 text-sm sm:text-base md:text-lg lg:text-xl font-roboto-400 text-hitamdikit"
                                    {...register("tanggalBeli", { required: "Tanggal beli wajib diisi", valueAsDate: true })}
                                />
                                {errors.tanggalBeli && (
                                    <p className="text-red-500 text-sm">{errors.tanggalBeli.message}</p>
                                )}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-xl p-10 mt-8">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-[18px] mt-6">
                            <div>
                                <label htmlFor="jumlah" className="text-sm sm:text-base md:text-lg lg:text-xl font-roboto-400 text-hitamdikit block mt-2">
                                ...
                                <div className="flex w-full border border-hitamdikit/30 rounded-xl overflow-hidden">
                                    <input
                                        ...
                                        {...register("jumlah", { required: "Jumlah wajib diisi", valueAsNumber: true })}
                                    />


                                    <select className="border border-hitamdikit/30 rounded-xl px-3 py-2 rounded-l-none font-roboto-400" {...register("satuan", { required: "Satuan wajib diisi" })}>
                    ...
                    <div className="bg-white rounded-xl p-10 shadow-lg mt-8">
                        ...
                        <div className="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-[18px] ">
                            {lokasiList.map((item) => (
                                ...
                                >
                                    <Image src={item.image} alt={item.name} width={66} height={58} />

                                    <h4 className="text-sm sm:text-base md:text-lg lg:text-xl xl:text-2xl font-roboto-400 text-hitamdikit">{item.name}</h4>
                                    <p className="text-sm sm:text-base md:text-lg lg:text-xl font-roboto-400 text-hitamdikit/50">{item.desc}</p>
                                </button>
                            ))}
                        ...
                        {selectedKategori && (
                            <div className="mt-6 p-6 bg-primaryskyblue flex flex-col sm:flex-row gap-3 items-center rounded-xl">
                                <TriangleAlert className="text-orangnormal bg-white rounded-xl p-2 w-[58px] h-[58px]" size={38} />
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-8">
                        <Button variant="primary" href="/bahan-saya" size="lg" className="w-full border-text-primary text-text-primary hover:shadow-lg">
                            Batalkan
                        </Button>
                        <Button variant="primary" size="lg" type="submit" className="w-full bg-text-primary text-white ">
