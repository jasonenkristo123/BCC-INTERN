...
import { filterItemsByMonth, getExpiryStatus, formatCurrency, formatDate } from '@/shared/utils/utils';
        ...
        <div className="mt-6 md:mt-8 px-4 sm:px-6 lg:px-8">
            <div className="flex flex-col gap-5 mb-6">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                    <span className="text-sm font-roboto-400 text-hitamdikit hidden sm:block">Kategori:</span>
                ...
                <div className="flex flex-col w-full divide-y divide-gray-50 lg:divide-y-0">
                    {paginatedItems.length > 0 ? (
                        paginatedItems.map((item, index) => {
                            const getBgColor = index % 2 !== 0 ? 'bg-linear-to-r from-bg-primaryskyblue to-white' : 'bg-white';
                            return (
                                <div key={item.id} className={`flex flex-col lg:flex-row lg:items-center px-4 sm:px-6 lg:px-8 py-5 lg:py-6 ${getBgColor} transition-colors duration-200 group`}>
                                    {/* Mobile labels */}
                                    <div className="flex items-center gap-4 lg:flex-[1.5] min-w-0">
                                        <div className="w-[48px] h-[48px] sm:w-[56px] sm:h-[56px] lg:w-[58px] lg:h-[58px] rounded-xl bg-white border border-gray-100 flex items-center justify-center overflow-hidden shrink-0 shadow-sm relative">
                                            {item.image ? (
                                                <Image src={item.image} width={50} height={36} alt={item.name} />
                                            ) : (
                                                <div className="w-full h-full bg-gray-100" />
                                            )}
                                        </div>
                                        <div className="flex flex-col min-w-0">
                                            <p className="font-roboto-600 text-sm sm:text-base text-hitamdikit truncate">{item.name}</p>
                                            <p className="font-roboto-400 text-xs sm:text-sm text-hitamdikit/60 truncate">Kategori {item.category}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-y-4 gap-x-4 mt-5 lg:mt-0 lg:flex lg:flex-3 lg:items-center w-full text-sm sm:text-base font-semibold text-hitamdikit">
                                        <div className="lg:flex-1 lg:text-center">
                                            <span className="lg:hidden text-xs text-hitamdikit/60 block mb-1 font-normal">Tanggal Dibuang</span>
                                            {formatDate(item.expiredEstimation)}
                                        </div>
                                        <div className="lg:flex-1 lg:text-center">
                                            <span className="lg:hidden text-xs text-hitamdikit/60 block mb-1 font-normal">Jumlah</span>
                                            {item.quantity}
                                        </div>
                                        <div className="col-span-2 sm:col-span-1 lg:flex-1 lg:text-center">
                                            <span className="lg:hidden text-xs text-hitamdikit/60 block mb-1 font-normal">Kerugian</span>
                                            <span className="text-red-500 lg:text-hitamdikit">{formatCurrency(item.price)}</span>
                                        </div>
                                    </div>
                                </div>
                            );
...
function ColHeader({ children }: { children: React.ReactNode }) {
    return <span className="text-[14px] font-bold font-roboto-600 text-hitamdikit">{children}</span>;
}