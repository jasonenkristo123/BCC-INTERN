...
export default function StatsSummary({ items }: StatsSummaryProps) {
    const expired = items.filter((i) => getExpiryStatus(i.expiredEstimation).status === "expired").length;
    const warning = items.filter((i) => getExpiryStatus(i.expiredEstimation).status === "warning").length;
