...
export default function RankingPanelCategory({
  ...
  return (
    <div className="flex items-center gap-2 sm:gap-4 p-3 sm:p-5 border border-gray-100 rounded-3xl shadow-sm bg-white mb-5 last:mb-0 transition-all hover:shadow-md">
      {/* Rank Circle */}
      <div
        className={`w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center font-roboto-500 font-bold sm:text-lg shrink-0 text-hitamdikit ${rankBgClass}`}
      >
        {rank}
      </div>
      ...
      <div className="flex items-center gap-3 w-[120px] sm:w-[150px] shrink-0">
        ...
        <div className="flex flex-col">
          <span className="font-roboto-500 text-hitamdikit font-roboto-600 text-sm md:text-base truncate max-w-[80px] sm:max-w-[100px]">
            {categoryName}
          </span>
          <span className="text-hitamdikit/50 text-[10px] sm:text-xs lg:text-sm font-roboto-400">
            {quantity}
          </span>
        </div>
      </div>
